...
Issue lifecycle 与 Issue policy 任务只在 `deepseek-harness/deepseek-harness` 中运行，因为只有该处具备对应的仓库配置与 GitHub App。下游仓库仅在 `DSH_ENABLE_REAL_API_E2E` 仓库变量为 `true` 时运行真实 API E2E；现有预检仍要求 `DEEPSEEK_API_KEY_EXTERNAL`，fork 与 Dependabot 拉取请求仍被排除。必需的 Linux 任务与独立原生 Windows 任务只在官方仓库选择其自定义 runner；下游仓库改用 `ubuntu-latest` 与 `windows-2025`，并限制工作进程数。无密钥静态检查、单元测试、快照、打包以及 Windows 与 macOS 检查因此无需组织专属 runner 标签即可运行。
...
- **要求下游维护者复刻上游自定义 runner 标签。** 否决：正确性验证必须能在标准 GitHub 托管容量上运行，官方仓库则可以继续使用速度更快的 runner 池。
...
- 下游拉取请求会比官方自定义 runner 消耗更多运行时间，但不会因组织专属标签不可用而永久排队。
