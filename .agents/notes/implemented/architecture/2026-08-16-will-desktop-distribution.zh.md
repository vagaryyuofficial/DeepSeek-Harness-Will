...
`apps/will-desktop` 是下游应用壳，不是新的核心 package。它在子进程中启动内置 `@deepseek-ai/dsh web` 入口，绑定 `127.0.0.1` 临时端口，再由启用上下文隔离的 Electron 窗口加载这一精确 origin。preload 只加入发行版界面：分平台原生窗口栏、皮肤、余额、`soul.md`、插件操作、持久原生终端、更新、路径、偏好和通知。原生模式会移除全部 Will CSS token，因此官方外观始终是默认值。
